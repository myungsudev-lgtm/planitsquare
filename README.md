...
가장 크게 배운 점은 버전 호환성 검증의 중요성입니다.
Spring Boot 3.x 환경에서는 외부 라이브러리(Swagger, QueryDSL 등)와의 연동이
버전 차이로 인해 쉽게 오류를 유발할 수 있으며, 실제로 실행·빌드·테스트 단계에서
여러 충돌을 경험했습니다.

특히 문제 해결 자체보다 원인이 버전 때문임을 찾는 과정이 더 오래 걸렸습니다.
따라서 외부 API나 라이브러리를 사용할 때는 다음 사항을 반드시 선행해야 합니다:

- 프로젝트에서 사용하는 Spring Boot 버전과 라이브러리의 공식 호환성 확인

- Gradle 설정 및 플러그인 버전의 정합성 유지

- 환경 버전을 초기 단계에서 명확히 정의하고 지속적으로 일관성 유지

이번 경험을 통해 버전 체크 → 기능 확인 → 프로젝트 적용이라는 절차가
개발 속도와 안정성을 크게 좌우한다는 것을 다시 확인할 수 있었습니다.
